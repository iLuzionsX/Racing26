...
export class Simulation {
  ...
  private interpolateState(prev: VehicleState, curr: VehicleState, alpha: number): VehicleState {
    ...
    return {
      ...
      // elevationHeight is the road datum added to heave in CarRenderer
      // (root Y) and subtracted for wheel Y (hub.y - elevationHeight).
      // Leaving it stepped while heave/hub are smoothed reintroduces
      // 120 Hz vertical quantization on graded/elevated track.
      elevationHeight: (() => {
        const p = (prev as any).elevationHeight;
        const c = (curr as any).elevationHeight;
        if (Number.isFinite(p) && Number.isFinite(c)) return lerp(p, c, alpha);
        return c;
      })(),
